...
$(document).on("change", "#raceSelect", (event) => {
  $("#principal").append(`
        <table class="table text-center"  id="resultTable">
            ...
            <button class="btn btn-success mt-1" name="guardar" value="guardar" id="guardar">Guardar</button>
        </table>
    `);
      ...
      response.forEach((e) => {
        ...
        $("#resultTable tbody").append(
          `
                        <tr>
                            <td>
                               <input class="form-control posiciones" type="text"  placeholder="Pos" value="${
                                 e.raceResultPosition
                               }"><input class="form-control raceResultID" value="${
            e.raceResultID
          }" type="text" hidden="hidden"></td>
                            <td>
                                <select id="driverID" name="driverID[]" class="form-select pilotos" aria-label="Default select example">
                                    <option selected value="${e.driverID}">${
            e.driverFirstName
          } ${e.driverLastName}</option>
                                </select>
                            </td>
                            <td><input class="form-control carNumber" type="text" name="carNumber[]" placeholder="#Num" value="${
                              e.carNumber
                            }"><input class="form-control carID" value="${
            e.carID
          }" type="text" name="carNumber[]" placeholder="#Num" hidden="hidden"></td>
                            <td><input class="form-control carBrand" type="text" name="carBrand[]" placeholder="Brand" value="${
                              e.carManufacturer
                            }"></td>
                            <td><input class="form-control laps" type="text" name="laps[]" placeholder="Laps" value="${
                              e.raceResultLaps
                            }"></td>
                            <td><input class="form-control gaps" type="text" name="gaps[]" placeholder="Gaps" value="${
                              e.raceResultGap
                            }"></td>
                            <td><input class="form-control points" type="text" name="points[]" placeholder="Puntos" value="${
                              e.raceResultPointsScored
                            }"></td>
                            <td><input class="form-control driverELO" type="text"  placeholder="Elo" value="${
                              e.driverELO - e.raceResultEloChanged
                            }"></td>
                            <td><input class="form-control eloUpdated" type="text"  placeholder="Gains" value="${
                              e.raceResultEloChanged
                            }"></td>
                        </tr>
                        `
        );
...
$(document).on("change", ".pilotos", (event) => {
  let selectedDriver = drivers.find(
    (element) => element.driverID === $(event.target).val()
  );
  ...
  $(event.target)
    ...
    .next()
    .children()
    .val(selectedDriver.carManufacturer);
  $(event.target)
    .parent()
    .next()
    .next()
...
$(document).on("click", "#guardar", (event) => {
  ...
  for (let i = 0; i < drivers.length; i++) {
    let eloTotal =
      parseInt($(".driverELO").eq(i).val()) +
      parseInt($(".eloUpdated").eq(i).val());
    $.ajax({
